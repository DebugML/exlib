...
class AmpeTextCls(FeatureAttrMethod):
    def __init__(self, model, projection_layer, data_min=0, data_max=1, epsilon=16, N=20, num_steps=10, 
                use_sign=True, use_softmax=True, verbose=False):
        super().__init__(model)
        self.data_min = data_min
        self.data_max = data_max
        self.epsilon = epsilon
    ...
    def forward(self, x, t, return_groups=False, **kwargs):
        ...
        with torch.enable_grad():
            ...
            def get_attr_fn(x_embed, t, x_kwargs, model, **kwargs):
                ...
                mfaba_results = torch.tensor(ampe(model, x_embed, t, **kwargs), device=x.device)
                # import pdb; pdb.set_trace()
                return mfaba_results.squeeze(1)

            attributions_all, _ = get_explanations_in_minibatches_text(x_embed, t, get_attr_fn, 
                mini_batch_size=16, show_pbar=False, 
                model=self.wrapped_model, data_min=self.data_min, data_max=self.data_max, epsilon=self.epsilon,
                N=self.N, num_steps=self.num_steps, use_sign=self.use_sign, 
                use_softmax=self.use_softmax, verbose=self.verbose)
